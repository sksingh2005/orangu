...
pub fn compress_shell_output(command: &str, output: &str) -> String {
    compress_shell_output_with_stats(command, output).0
}

pub fn compress_shell_output_with_stats(command: &str, output: &str) -> (String, CompressionStats) {
    ...
    let (compressed, pattern) =
            ...
            || normalised.contains("gradlew test")
        {
            ...
            (compress_git_diff(output), Some("git_diff".to_string()))
...
pub fn prepare_llm_diff_context(diff: &str, compression_enabled: bool) -> LlmContext {
    ...
    let compressed = compress_git_diff(diff);
    let compressed_lines = compressed.lines().count();
...
fn compress_git_diff(output: &str) -> String {
    ...
    let mut result = lines[..200].join("\n");
    let omitted = lines.len() - 250;
    result.push_str(&format!("\n\n... {} lines omitted ...\n\n", omitted));
    result.push_str(&lines[lines.len() - 50..].join("\n"));
...
test result: FAILED. 2 passed; 1 failed; 0 ignored; 0 measured; 0 filtered out\n";
    ...
    fn llm_diff_context_keeps_small_diffs_raw() {
        ...
        let context = prepare_llm_diff_context(diff, true);
    ...
    fn llm_diff_context_marks_large_compressed_diffs() {
        let diff = (0..600)
            ...
            .join("\n");
        let context = prepare_llm_diff_context(&diff, true);
        assert!(context.content.contains("lines omitted"));
        assert!(context.note.expect("note").contains("600 ->"));
    }
    ...
    fn llm_diff_context_can_be_disabled() {
        let diff = (0..600)
            ...
            .collect::<Vec<_>>()
            .join("\n");
        let context = prepare_llm_diff_context(&diff, false);
        assert_eq!(context.content, diff);
        assert!(context.note.is_none());
    }
    ...
    fn dispatch_env_wrapped_cargo_build() {
        ...
        let (_, stats) =
            compress_shell_output_with_stats("CARGO_TERM_COLOR=always cargo build", output);
    ...
    fn dispatch_time_wrapped_cargo_test() {
        ...
        let (_, stats) = compress_shell_output_with_stats("time cargo test", output);
