...
- Local commands such as `/help`, `/model`, `/server`, `/list_files`, `/show_file`, `/tools`, `/diff`, `/status`, `/log`, `/pull`, `/rebase`, `/merge`, `/checkout`, `/add_file`, `/remove_file`, `/move_file`, `/cherry_pick`, `/comment`, `/close`, `/get_comments`, `/commit`, `/amend`, `/push`, `/review`, `/init_repo`, `/squash`, `/delete`, and `/open_file`
